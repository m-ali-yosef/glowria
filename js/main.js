(function () {
  ...
  /* تم مسح السعر وكلمة / bottle من البطاقات الخارجية لزيادة الـ Engagement */
  ...
  /* يظهر السعر والحجم داخل صفحة تفاصيل المنتج فقط */
  ...
  /* تحديث تلقائي لرقم الهاتف من SITE.phone في الجافاسكربت */
  function updatePhoneNumbers() {
    if (typeof SITE !== "undefined" && SITE.phone) {
      $$("li, p, strong").forEach((el) => {
        if (el.children.length === 0 && el.textContent.includes("+")) {
          el.textContent = el.textContent.replace(/\+\d[\d\s]+/, SITE.phone);
        }
      });
    }
  }
